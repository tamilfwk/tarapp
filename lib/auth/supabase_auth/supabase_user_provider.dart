...
class TaraiSupabaseUser extends BaseAuthUser {
  TaraiSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> taraiSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TaraiSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
